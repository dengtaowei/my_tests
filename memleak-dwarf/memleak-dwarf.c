...
static void show_stack_trace(char *buffer, int buflen, int *pos, __u64 *stack, int stack_sz, pid_t pid)
{
    ...
    {
        Dwfl_Module *mod = dwfl_addrmodule(dwfl, stack[i]);
        if (!mod) {
            fprintf(stderr, "No module contains address 0x%llx\n", stack[i]);
            break;
        }

        const char *name = dwfl_module_addrname(mod, stack[i]);
        if (!name) name = "[Unknown]";

        Dwarf_Addr bias;
        Dwarf_Die *cudie = dwfl_module_addrdie(mod, stack[i], &bias);
        if (cudie) {
            Dwarf_Line *line = dwarf_getsrc_die(cudie, stack[i] - bias);
            if (line) {
                const char *src_file = dwarf_linesrc(line, NULL, NULL);
                int line_num;
                if (dwarf_lineno(line, &line_num) == 0) {
                    *pos += snprintf(buffer + *pos, buflen - *pos, "0x%llx Func %s (%s:%d)\n", stack[i], name, src_file, line_num);
                } else {
                    *pos += snprintf(buffer + *pos, buflen - *pos, "0x%llx Func %s (%s:Unknown)\n", stack[i], name, src_file);
                }
            } else {
                *pos += snprintf(buffer + *pos, buflen - *pos, "0x%llx Func %s (No debug info)\n", stack[i], name);
            }
        } else {
            *pos += snprintf(buffer + *pos, buflen - *pos, "0x%llx Func %s (No DWARF info)\n", stack[i], name);
        }
    }
out:
...
int print_outstanding_combined_allocs(struct memleak_dwarf_bpf * skel, pid_t pid)
{

	return 0;
}

int attach_uprobes(struct memleak_dwarf_bpf *skel)
{
...
int main(int argc, char **argv)
{
	...
	err = memleak_dwarf_bpf__load(skel);
	if (err) {
	 ...
	 */
	err = memleak_dwarf_bpf__attach(skel);
	if (err) {
...
cleanup:
	...
	memleak_dwarf_bpf__destroy(skel);
#if 0
