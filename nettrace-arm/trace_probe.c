...
static struct nettrace_bpf *skel;
...
again:
	...
	if (!auto_attach && nettrace_bpf__attach(skel)) {
...
static int probe_trace_load()
{
	...
	skel = nettrace_bpf__open_opts(&opts);
	if (!skel) {
	...
	if (trace_pre_load() || nettrace_bpf__load(skel)) {
...
{
	struct nettrace_bpf *tmp = nettrace_bpf__open();
	struct bpf_program *prog;
	int err;
	...
	err = nettrace_bpf__load(tmp);
	nettrace_bpf__destroy(tmp);
